/**
 * Velox AI Chatbot Widget v1.0
 * Usage: <script src="velox-widget.js" data-config='{"bizName":"...","accentColor":"...","faq":"...","tone":"...","industry":""}'></script>
 */
(function () {
  ...
  style.textContent = `
    ...
    #vlx-fab .vlx-badge.visible { display: flex; }
    ...
    #vlx-header {
      ...
      background: ${accentColor}; color: white; flex-shrink: 0;
    }
    #vlx-avatar {
    ...
    #vlx-messages {
      ...
      flex-direction: column; gap: 10px; min-height: 220px; max-height: 300px;
      background: #f8f9fb;
    }
    ...
    .vlx-bubble {
      padding: 9px 13px; border-radius: 16px; font-size: 13px; line-height: 1.55;
  ...
  root.innerHTML = `
    ...
    <div id="vlx-window" role="dialog" aria-label="Chat with ${bizName}">
      <div id="vlx-header">
        ...
        <button id="vlx-close-btn" aria-label="Close chat">✕</button>
      </div>
  ...
  setTimeout(() => {
    if (!isOpen) { badge.classList.add('visible'); hasShownBadge = true; }
  }, 4000);
  ...
  function initChat() {
    msgs.innerHTML = '';
    history = [];
    const clean = greeting.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>');
    addBotBubble(clean);
    renderQR();
  }

  function renderQR() {
    qrBox.innerHTML = quickReplies.map(r =>
      `<button class="vlx-qr">${r}</button>`
    ).join('');
    qrBox.querySelectorAll('.vlx-qr').forEach(b =>
  ...
  root.querySelector('#vlx-lf-btn').addEventListener('click', () => {
    ...
    addBotBubble(`Thanks <strong>${name}</strong>! ✅ We've got your details and someone will be in touch soon. Anything else I can help with?`);
    history.push({ role: 'assistant', content: `Lead captured for ${name} (${contact})` });
    renderQR();
    // In production: POST to your backend here
    console.log('[Velox AI] Lead captured:', { name, contact, note, business: bizName, ts: new Date().toISOString() });
  });
  ...
  async function send(text) {
    ...
    const systemPrompt = `You are a helpful AI assistant chatbot for "${bizName}", a ${industry} business.
Tone: ${tone}
Key business info:
${faq}
Rules:
- Keep replies SHORT (2–4 sentences). Be direct and warm.
- If someone wants to book, get a quote, or enquire, output exactly: [COLLECT_LEAD]
- If asked something you don't know, say you'll get someone from the team to follow up, then output: [COLLECT_LEAD]
...
- Stay in character as a representative of ${bizName}.`;
    ...
    try {
      ...
      if (reply.includes('[COLLECT_LEAD]')) {
        ...
        if (clean) { addBotBubble(clean); history.push({ role:'assistant', content: clean }); }
        showLeadForm();
      } else {
        addBotBubble(reply.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>'));
  ...
  textInput.addEventListener('keydown', e => {
    if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); send(textInput.value.trim()); }
  });
